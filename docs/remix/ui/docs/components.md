...
2. **Subsequent Updates**:
   ...
   - Component phase is skipped, and the closure persists for the lifetime of
...
3. **Component Removal**:
   ...
   - All event listeners registered via `addEventListeners()` are automatically
     cleaned up
   - Any queued tasks are executed with an aborted signal

## Props On The Handle

Props are available on `handle.props`. The object is stable, and its values are
updated before each render:
