...
- Add first-class migration execution support to the postgres adapter. It now
...
- Add transaction-aware migration introspection to the postgres adapter.
  ...
  now use the provided migration transaction client when present, so planning
  and execution can inspect schema state inside the active migration
  transaction.
