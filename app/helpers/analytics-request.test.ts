...
describe('analytics-request helpers', () => {
	...
	test('returns null fingerprint across repeated Forwarded invalid-value matrix without user-agent', () => {
		for (const buildHeader of repeatedForwardedForHeaderBuilders) {
			for (const firstValue of repeatedForwardedInvalidValues) {
				for (const secondValue of repeatedForwardedInvalidValues) {
					const repeatedHeader = buildHeader(firstValue, secondValue)
	...
	test('falls back to X-Real-IP across repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.250'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
	...
	test('prefers X-Forwarded-For and preserves unknown user-agent across repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.230'
		const userAgent = 'CustomPodClient/1.2 (Linux)'
		const expectedClientName = 'CustomPodClient/1.2'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
		...
		for (const buildHeader of repeatedForwardedForHeaderBuilders) {
			for (const firstValue of repeatedForwardedInvalidValues) {
				for (const secondValue of repeatedForwardedInvalidValues) {
					for (const invalidXRealIp of crossHeaderInvalidXRealIpValues) {
						const repeatedHeader = buildHeader(firstValue, secondValue)
						const request = new Request('https://example.com/media', {
							headers: {
								Forwarded: repeatedHeader,
								'X-Forwarded-For': `unknown, ${expectedIp}:443`,
								'X-Real-IP': invalidXRealIp,
								'User-Agent': userAgent,
							},
						})
	...
	test('prefers X-Forwarded-For and preserves known user-agent classification across repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.229'
		const userAgent = 'Pocket Casts/7.58'
	...
	test('falls back to X-Real-IP when repeated Forwarded and X-Forwarded-For are invalid', () => {
		const expectedIp = '198.51.100.248'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
				'X-Real-IP': expectedIp,
			},
	...
	test('uses unknown user-agent fallback when repeated Forwarded and other proxy headers are invalid', () => {
		const userAgent = 'CustomPodClient/1.2 (Linux)'
		const expectedClientName = 'CustomPodClient/1.2'
		const userAgentOnlyRequest = new Request('https://example.com/media', {
			headers: {
	...
	test('returns null metadata when repeated Forwarded and other proxy headers are invalid without user-agent', () => {
		for (const buildHeader of repeatedForwardedForHeaderBuilders) {
			for (const firstValue of repeatedForwardedInvalidValues) {
				for (const secondValue of repeatedForwardedInvalidValues) {
					for (const invalidXForwardedFor of crossHeaderInvalidXForwardedForValues) {
	...
	test('returns null fingerprint across triple repeated Forwarded invalid-value matrix without user-agent', () => {
		for (const buildHeader of repeatedForwardedTripleForHeaderBuilders) {
			for (const firstValue of repeatedForwardedInvalidValues) {
				for (const secondValue of repeatedForwardedInvalidValues) {
					for (const thirdValue of repeatedForwardedInvalidValues) {
	...
	test('falls back to X-Real-IP across triple repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.251'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
				'X-Real-IP': expectedIp,
			},
	...
	test('prefers X-Forwarded-For and preserves unknown user-agent across triple repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.231'
		const userAgent = 'CustomPodClient/1.2 (Linux)'
		const expectedClientName = 'CustomPodClient/1.2'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
	...
	test('prefers X-Forwarded-For and preserves known user-agent classification across triple repeated Forwarded invalid-value matrix', () => {
		const expectedIp = '198.51.100.228'
		const userAgent = 'Pocket Casts/7.58'
		const expectedClientName = 'Pocket Casts'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
	...
	test('falls back to X-Real-IP when triple repeated Forwarded and X-Forwarded-For are invalid', () => {
		const expectedIp = '198.51.100.249'
		const canonicalRequest = new Request('https://example.com/media', {
			headers: {
				'X-Real-IP': expectedIp,
			},
	...
	test('uses user-agent fallback when triple repeated Forwarded and other proxy headers are invalid', () => {
		const userAgent = 'Pocket Casts/7.58'
		const userAgentOnlyRequest = new Request('https://example.com/media', {
			headers: {
				'User-Agent': userAgent,
	...
	test('uses unknown user-agent fallback when triple repeated Forwarded and other proxy headers are invalid', () => {
		const userAgent = 'CustomPodClient/1.2 (Linux)'
		const expectedClientName = 'CustomPodClient/1.2'
		const userAgentOnlyRequest = new Request('https://example.com/media', {
			headers: {
	...
	test('returns null metadata when triple repeated Forwarded and other proxy headers are invalid without user-agent', () => {
		for (const buildHeader of repeatedForwardedTripleForHeaderBuilders) {
			for (const firstValue of repeatedForwardedInvalidValues) {
				for (const secondValue of repeatedForwardedInvalidValues) {
					for (const thirdValue of repeatedForwardedInvalidValues) {
	...
	const crossHeaderExpectedXForwardedForIps = new Map<
		string | null,
		string | null
	>()
	const crossHeaderExpectedForwardedIps = new Map<
		string | null,
		string | null
	>()
	const crossHeaderExpectedXRealIpIps = new Map<string | null, string | null>()

	for (const headerValue of crossHeaderXForwardedForValues) {
		if (headerValue === null) {
			crossHeaderExpectedXForwardedForIps.set(headerValue, null)
			continue
		}
		crossHeaderExpectedXForwardedForIps.set(
			headerValue,
			getClientIp(
				new Request('https://example.com/media', {
					headers: {
	...
	for (const headerValue of crossHeaderForwardedValues) {
		if (headerValue === null) {
			crossHeaderExpectedForwardedIps.set(headerValue, null)
			continue
		}
		crossHeaderExpectedForwardedIps.set(
			headerValue,
			getClientIp(
				new Request('https://example.com/media', {
					headers: {
	...
	for (const headerValue of crossHeaderXRealIpValues) {
		if (headerValue === null) {
			crossHeaderExpectedXRealIpIps.set(headerValue, null)
			continue
		}
		crossHeaderExpectedXRealIpIps.set(
			headerValue,
			getClientIp(
				new Request('https://example.com/media', {
					headers: {
	...
	function forEachCrossHeaderCombination(
		callback: (
		...
		for (const xForwardedForValue of crossHeaderXForwardedForValues) {
			for (const forwardedValue of crossHeaderForwardedValues) {
				for (const xRealIpValue of crossHeaderXRealIpValues) {
					...
					const expectedIp =
						crossHeaderExpectedXForwardedForIps.get(xForwardedForValue) ??
						crossHeaderExpectedForwardedIps.get(forwardedValue) ??
						crossHeaderExpectedXRealIpIps.get(xRealIpValue) ??
						null

					callback(headers, expectedIp)
				}
			}
		}
	}
	...
	test('preserves cross-header precedence across segment combination matrix', () => {
		...
		forEachCrossHeaderCombination((headers, expectedIp) => {
			const request = new Request('https://example.com/media', {
				headers: {
					...headers,
					'User-Agent': userAgent,
	...
	test('preserves cross-header precedence across segment combination matrix without user-agent', () => {
		forEachCrossHeaderCombination((headers, expectedIp) => {
			const request = new Request('https://example.com/media', { headers })
