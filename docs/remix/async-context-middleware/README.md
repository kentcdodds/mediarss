...
Request-scoped async context middleware for Remix. It stores each request
context in
[`AsyncLocalStorage`](https://nodejs.org/api/async_context.html#class-asynclocalstorage)
so utilities can access it anywhere in the same async call stack.

## Features

- **Request context access** - Read the current `RequestContext` from anywhere
  in the same async execution flow
- **App-typed `getContext()`** - Augment `AsyncContextTypes` so `getContext()`
  returns your app's request context contract
...
Use `asyncContext()` at the router level to make the current request context
available to helpers deeper in the same async call stack.
...
This middleware requires support for `node:async_hooks`, so it is intended for
Node.js runtimes.

## Typed `getContext()`

`getContext()` is global and out-of-band, so apps can augment
`AsyncContextTypes` to tell the package what request context lives in async
local storage.
...
After that augmentation, `getContext()` returns `AppContext<AnyParams>`
...
Use a broad app-level context like `AppContext<AnyParams>` here. Route handlers
themselves can still use more precise route-specific params in their own
...
- [`auth-middleware`](https://github.com/remix-run/remix/tree/main/packages/auth-middleware) -
  Request-time auth state and protected route middleware
- [`session-middleware`](https://github.com/remix-run/remix/tree/main/packages/session-middleware) -
  Session loading middleware often paired with async request context
