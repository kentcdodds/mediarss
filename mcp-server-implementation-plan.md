...
This document outlines the plan for implementing a Model Context Protocol (MCP)
server in this project, based on the
[MCP Authorization specification (2025-11-25)](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization)
and insights from the
[MCP Auth workshop](https://github.com/epicweb-dev/mcp-auth).
...
The existing `/.well-known/oauth-authorization-server` endpoint already exists
at `app/routes/oauth/server-metadata.ts`. No changes needed, but verify it
includes:
...
request/response objects. We need to create an adapter for Bun's native HTTP
handling.
...
**Option A: Node.js Compatibility Layer** Use Bun's built-in Node.js
compatibility to wrap requests/responses.

**Option B: Custom Bun Transport** Create a new transport class based on
`StreamableHTTPServerTransport` logic that works natively with Web API
Request/Response.

**Option C: Fetch Handler Adapter** Create an adapter that converts between Web
API and Node.js stream interfaces.

Recommended approach: **Option B** - Create a `BunStreamableHTTPServerTransport`
class that:
...
Modify the authorize endpoint to accept and validate MCP scopes when an MCP
...
3. **MCP Inspector** Use the MCP Inspector tool to test the full flow:
   ```bash
...
1. **Token Validation**: Always verify JWT tokens using our existing OAuth
   infrastructure
...
1. **Token Introspection Endpoint**: Add `/oauth/introspection` for RFC 7662
   compliance
...
4. **Resource Subscriptions**: Implement real-time updates for subscribed
   resources
