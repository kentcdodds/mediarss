...
HTTP proxy utilities built on the web
[Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API). Use
`fetch-proxy` to create `fetch` handlers that forward requests to target servers
while optionally rewriting headers and cookies.

## Features

- **Web Standards** - Built on the standard
  [JavaScript Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API)
- **Cookie Rewriting** - Supports rewriting `Set-Cookie` headers received from
  target server
- **Forwarding Headers** - Supports `X-Forwarded-Proto` and `X-Forwarded-Host`
  headers
...
- [`node-fetch-server`](https://github.com/remix-run/remix/tree/main/packages/node-fetch-server) -
  Build HTTP servers for Node.js using the web fetch API
