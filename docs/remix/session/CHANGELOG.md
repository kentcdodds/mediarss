...
[`session`](https://github.com/remix-run/remix/tree/main/packages/session). It
...
- Add `Session` class. The `createSession` function now returns an instance of
  the `Session` class.
...
- BREAKING CHANGE: Rename `createFileSessionStorage` to `createFsSessionStorage`
  and export from `@remix-run/session/fs-storage`
...
- Fix flash messages persisting across multiple requests. Flash data is now
  automatically cleared after being available for one request, even if the
  session is not otherwise modified

## v0.2.0 (2025-11-18)

- BREAKING CHANGE: Remove `Session` class, use `createSession` instead
- BREAKING CHANGE: Remove class versions of session storage, use the factory
  functions instead
...
- Add `session.regenerateId(deleteOldSession?: boolean)` to purge old session
  data when the session ID is regenerated. This is useful for preventing session
  fixation attacks.
...
See the
[README](https://github.com/remix-run/remix/blob/main/packages/session/README.md)
for more details.
