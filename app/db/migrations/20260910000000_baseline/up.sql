-- Baseline: the complete schema as of legacy migration version 8.
--
-- Every statement is idempotent so this migration adopts databases created by
-- the previous hand-rolled migration runner (which journaled in
-- `schema_versions`) as well as brand-new databases.
...
CREATE TABLE IF NOT EXISTS directory_feeds (
	...
	subtitle TEXT,
	copyright TEXT,
	feed_type TEXT CHECK (feed_type IN ('episodic', 'serial'))
);
...
CREATE INDEX IF NOT EXISTS idx_feed_analytics_events_feed_id_created_at ON feed_analytics_events(feed_id, created_at);
CREATE INDEX IF NOT EXISTS idx_feed_analytics_events_token_created_at ON feed_analytics_events(token, created_at);
