...
There is no build step. This is shipped as-is. The server runs TypeScript
directly via `node --import remix/node-tsx` (`.ts` and `.tsx`, JSX configured in
`tsconfig.json`). Browser code is served on demand by the `remix/assets` asset
server configured in `app/assets.ts` and `remix.json` (fingerprinted in
production, with import maps generated by `getScriptEntry()`). `npm run dev`
supervises the server with `remix/node-hmr` for server and Remix UI hot updates.
...
`node_modules` package used by browser code must be listed in
`assets.allowPackages` in `remix.json` (and must ship ESM; CommonJS-only
packages need an ESM shim under `app/client/vendor/` marked as
`scripts.external` in `app/assets.ts`).
