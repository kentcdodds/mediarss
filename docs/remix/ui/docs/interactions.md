...
Build reusable event behavior with mixins that compose normal DOM events into
semantic custom events.

> **Note:** Most app code should stick with `on('click', ...)` and other native
> events. Reach for custom event mixins when the behavior is complex and reused
> in multiple places.
...
4. Use `handle.addEventListener('remove', ...)` for timer/listener cleanup (GC
   will take care of scoped variables).
