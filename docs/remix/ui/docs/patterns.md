...
function CacheExample(
	handle: Handle<{ cacheSize: number; key: string; value: any }>,
) {
...
```tsx
function Analytics(
	handle: Handle<{ apiKey: string; event: string; data?: any }>,
) {
	// Initialize SDK once
...
Note that by fetching this data in the component scope, parent updates that
change `userId` will not restart the request unless you add that behavior
yourself.
