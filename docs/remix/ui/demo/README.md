...
A focused demo for the `@remix-run/ui` package using the built-in `RMX_01`
preset.

## What It Shows

- A practical, utilitarian web app theme instead of a marketing landing page
  look
- Shared semantic tokens applied across buttons, cards, popover-like surfaces,
  and content areas
- How `theme`, flat button `*Style` exports, and component-owned namespaces
  compose together in Remix component code
...
The demo includes a Playwright-based review workflow so you can capture the
current UI without manually taking screenshots.
...
- `@remix-run/ui` entrypoints for `RMX_01`, `theme`, `button`, and component
  primitives
