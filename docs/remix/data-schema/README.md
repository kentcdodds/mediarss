...
Map a validated value into the shape your app wants. The transformer runs after
the schema validates and changes the parsed output type.
...
Use `.refine()` for checks that reject values without changing them. Use
`.transform()` for safe, synchronous mappings; thrown errors are propagated
instead of converted into validation issues.
