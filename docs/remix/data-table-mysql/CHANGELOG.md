...
- Add first-class migration execution support to the mysql adapter. It now
  ...
  flows, migration journal writes, and adapter-managed migration locking.
...
- Add transaction-aware migration introspection to the mysql adapter.
  ...
  now use the provided migration transaction connection when present, so
  planning and execution can inspect schema state inside the active migration
  transaction.
