...
For button-like actions, prefer native interactive elements and listen for
`click`. Buttons already handle pointer, touch, Enter, and Space activation
consistently.
...
For custom non-button controls, add explicit keyboard handling for the
activation keys your control supports.
