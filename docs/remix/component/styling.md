...
The `css(...)` mixin provides inline styling with support for pseudo-selectors,
...
The `css(...)` mixin produces static styles that are inserted into the document
as CSS rules, while the `style` prop applies styles directly to the element. For
...
function CardWithJSState(handle: Handle) {
	...
	return (props: { children: RemixNode }) => (
		...
		>
			<div
				className="title"
				mix={[css({ color: isHovered ? 'blue' : '#333' })]}
			>
				Title
			</div>
		</div>
