...
test('feed route uses unknown user-agent fallback across repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route prefers X-Forwarded-For across repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.240'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
...
test('feed route prefers X-Forwarded-For and preserves known user-agent classification across repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.241'
	const userAgent = 'Pocket Casts/7.58'
...
test('feed route falls back to X-Real-IP when repeated Forwarded and X-Forwarded-For are invalid', async () => {
	...
	const expectedIp = '198.51.100.233'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
			'X-Real-IP': expectedIp,
		},
...
test('feed route falls back to X-Real-IP and preserves unknown user-agent across repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route falls back to X-Real-IP and preserves known user-agent classification across repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.222'
	const userAgent = 'Pocket Casts/7.58'
	const expectedClientName = 'Pocket Casts'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
...
test('feed route uses user-agent fallback when repeated Forwarded and other proxy headers are invalid', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route uses unknown user-agent fallback across triple repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route prefers X-Forwarded-For across triple repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route prefers X-Forwarded-For and preserves known user-agent classification across triple repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.245'
	const userAgent = 'Pocket Casts/7.58'
	const expectedClientName = 'Pocket Casts'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
...
test('feed route falls back to X-Real-IP when triple repeated Forwarded and X-Forwarded-For are invalid', async () => {
	...
	const expectedIp = '198.51.100.234'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
			'X-Real-IP': expectedIp,
		},
...
test('feed route falls back to X-Real-IP and preserves unknown user-agent across triple repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.221'
	const userAgent = 'CustomPodClient/1.2 (Linux)'
	const expectedClientName = 'CustomPodClient/1.2'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
...
test('feed route falls back to X-Real-IP and preserves known user-agent classification across triple repeated Forwarded invalid-value matrix', async () => {
	using ctx = createDirectoryFeedRouteTestContext()

	const expectedIp = '198.51.100.220'
	const userAgent = 'Pocket Casts/7.58'
	const expectedClientName = 'Pocket Casts'
	const canonicalRequest = new Request('https://example.com/feed', {
		headers: {
...
test('feed route uses user-agent fallback when triple repeated Forwarded and other proxy headers are invalid', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
...
test('feed route applies precedence matrix with unknown user-agent tokenization', async () => {
	using ctx = createDirectoryFeedRouteTestContext()
	const userAgent = 'CustomPodClient/1.2 (Linux)'
	const expectedClientName = 'CustomPodClient/1.2'
	const cases = [
