...
// isSortingByPubDate tests

test('isSortingByPubDate returns true for pubDate sort field', () => {
	expect(isSortingByPubDate('pubDate')).toBe(true)
})

test('isSortingByPubDate returns true for publicationDate sort field', () => {
	expect(isSortingByPubDate('publicationDate')).toBe(true)
})

test('isSortingByPubDate returns true for pubDate with secondary sort fields', () => {
	expect(isSortingByPubDate('pubDate,title')).toBe(true)
	expect(isSortingByPubDate('publicationDate,author')).toBe(true)
})

test('isSortingByPubDate returns false for trackNumber sort field', () => {
	expect(isSortingByPubDate('trackNumber')).toBe(false)
})

test('isSortingByPubDate returns false for filename sort field', () => {
	expect(isSortingByPubDate('filename')).toBe(false)
})

test('isSortingByPubDate returns false for title sort field', () => {
	expect(isSortingByPubDate('title')).toBe(false)
})

test('isSortingByPubDate returns false for position sort field (curated feeds)', () => {
	expect(isSortingByPubDate('position')).toBe(false)
})

test('isSortingByPubDate returns false when pubDate is a secondary sort field', () => {
...
// getSyntheticPubDate tests

test('getSyntheticPubDate returns January 1, 1990 for index 0', () => {
...
test('getSyntheticPubDate returns January 2, 1990 for index 1', () => {
...
test('getSyntheticPubDate increments by one day for each index', () => {
...
test('getSyntheticPubDate maintains correct order for many items', () => {
...
test('getSyntheticPubDate handles large indices (thousands of items)', () => {
...
test('getSyntheticPubDate all dates are in the past', () => {
	const now = new Date()
